...
- `tracker/project_model.py` / `tracker/project_validation.py` / `tracker/project_query.py` / `tracker/project_mutation.py` / `tracker/subtask_templates.py` / `tracker/project_map.py`：已拆出的纯模型、校验、查询、状态机、子任务模板、地图视图层
...
- 将 `core.py` 中的共享常量、纯模型逻辑、校验逻辑、状态查询逻辑、状态机规则、子任务模板编排逻辑、地图视图逻辑拆到独立模块
