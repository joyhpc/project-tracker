...
- 健康检查/doctor 类命令必须严格只读：只能解析、汇总、报告，不得迁移、修复、生成报告或写回项目文件。
- 自然语言入口必须保持“解析预览”和“写入执行”分离；`--dry-run` 不得写入，`--json` 只表示输出格式，不得暗中改变执行语义。
- 仓库边界检查模块必须保持纯函数形态，只接收路径并返回违规结果，不得反向依赖 `core`、active project 或项目数据模型。
